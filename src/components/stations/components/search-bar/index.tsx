...
import { ActiveCell, LiveCell } from "common-components";
...
const SearchBar: React.FC<Props> = ({
  ...
  const handleSearch = (e: any) => {
    ...
    get(`/identity/station/list/?search=${e.target.value}`).then((res: any) => {
      userData1 = res.data.results;
      if (typed) {
        typed(e.target.value);
      }
      if (event.length !== 0) {
        const arr = JSON.stringify(userData1, function(key, value) {
          return value || false;
        });
