...
import { Pagination, HeaderLink, HeaderButton, SearchBar, FilterToolBar, Modal } from 'common-components';
...
const UserLanding = () => {
	...
	const renderPagination: Function = () => {
		return (
			<React.Fragment>
				<Divider />
				<Pagination paginateFn={paginate} totalItems={users.length} itemsPerPage={10} />
			</React.Fragment>
		);
	};
		...
		return (
			<div className={classes.header}>
				<HeaderLink
					...
					title="Users"
				/>
				<HeaderButton noIcon={false} buttonText="New User" openFunction={() => setIsNewUser(true)} />
			</div>
		);
	};
	...
	return (
		<React.Fragment>
			{state.companies.length !== 0 && (
				<NewUser open={is_new_user} setOpen={setIsNewUser} onClose={() => setIsNewUser(false)} />
			)}

			{loadingModal ? (
				<Modal open={loadingModal} onClose={() => setLoadingModal(false)} title="Edit user">
					<div
						style={{
							textAlign: 'center',
							height: '250px',
