			...
			<InputField
					...
					if (e.target.value) {
						dispatch({
							type: 'GROUP_STATE', payload: {
								groupState: { ...state.groupState, errMsg: '', name: e.target.value }
							}
						})
					} else {
						dispatch({
							type: 'GROUP_STATE', payload: {
								groupState: { ...state.groupState, errMsg: 'Option group name is required', name: e.target.value }
							}
						})
					}
				}}
				...
				onBlur={e => {
					if (e.target.value) {
						...
						dispatch({
							type: 'GROUP_STATE', payload: {
								groupState: { ...state.groupState, errMsg: 'Option group name is required' }
							}
						})
					}
				}}
