...
const OptionGroup = ({ history }: any) => {
  ...
  const fetchData = () => {
    setLoading(true);
    get("/pitch/global/gui/field-option-group/").then((res: any) => {
  ...
  const paginate = (from: number, to: number) => {
    dispatch({
      type: "GROUP_LIST",
      payload: { groupList: state.paginateList.slice(from, to) }
    });
  };
  ...
  return (
    <>
      ...
      <div className="option-header-container">
        <HeaderLink menu={menu} title="Option Group" />
        {state.groupList.length !== 0 ? (
          <HeaderButton
            openFunction={() =>
              dispatch({
                type: "GROUP_STATE",
                payload: { groupState: { ...state.groupState, create: true } }
              })
            }
              ...
              <SaveButton
                onClick={() =>
                  dispatch({
                    type: "GROUP_STATE",
                    payload: {
                      groupState: { ...state.groupState, create: true }
                    }
                  })
                }
              >
      ...
      <NewOptionGroup closeCreate={handleClose} createFn={handleCreate} />
      <LoadingModal
