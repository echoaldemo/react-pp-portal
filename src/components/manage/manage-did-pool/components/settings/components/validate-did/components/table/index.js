...
const Table = props => {
  ...
  async function fetchFullData(inv) {
  ...
  async function fetchInvalid() {
    ...
    if (response.status < 300) {
      ...
      let data = await response.data.map(async inv => {
  ...
  function renderTable(data, row, userCell) {
    let tblData = data.map(key => {
      return {
        number: key.number,
