...
export default function EditDIDModal(props) {
  const classes = useStyles()
  const [did, setDID] = React.useState(null)
  const [loading, setLoading] = React.useState(true)
  const [didPools, setPools] = React.useState(null)
  ...
  function getDIDPools() {
    if (didPoolsLoading) {
      get('/did/company/all/campaign/all/pool/').then(res => {
        setPools(res.data)
  ...
  React.useEffect(() => {
    ...
    if (props.editData !== null) {
      ...
      if (props.editData.pool !== null) {
        ...
        get(`/identity/campaign/${props.editData.pool.campaign}/`).then(res =>
          setCampaign(res.data)
        )
        get(`/identity/company/${props.editData.pool.company}/`).then(res =>
          setCompany(res.data)
        )
      }
    }
