...
class UserSettings extends Component {
    ...
    async componentDidMount() {
        //get all users
        const data = await get("/identity/user/manage/list/", { editable: true, limit: 1000 }).then(users => {
            this.setState({ userList: users.data.results })
        })
        const data2 = await get(`/identity/user/manage/list/`, { team: this.props.team }).then(members => {
    ...
    addToQueue = user => {
        if (!this.state.userQueue.some(u => u.uuid === user.uuid) &&
            !this.state.teamMembers.some(u => u.uuid === user.uuid)) {
            this.setState({
    ...
    addToTeam = () => {
        ...
        queue.map(user => {
            patch(
                `/identity/user/manage/${user.uuid}/`,
    ...
    removeMembers = () => {
        ...
        var filtered = this.state.teamMembers.filter(user => {
