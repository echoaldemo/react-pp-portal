...
export default function Variables() {
  ...
  const renderAddVarModal = () => (
    <AlertModal
      open={openAddVarModal}
      severity={"info"}
      message={"Informative text for example…"}
      handlerClickBtn={() => setOpenAddVarModal(!openAddVarModal)}
    />
  );
  ...
  const renderTableNoResult = () => (
    <TableNoResult
      ...
      renderButton={
        <SaveButton onClick={() => setOpenAddVarModal(!openAddVarModal)}>
          <Add />
          New Variable
        </SaveButton>
      }
    ...
    <React.Fragment>
      ...
      <AsyncTable
        ...
        render={(samples: any, { row, cell, uuid, icon }: any) =>
          samples.map(
            (sample: { uuid: string; name: string; slug: string }) => (
