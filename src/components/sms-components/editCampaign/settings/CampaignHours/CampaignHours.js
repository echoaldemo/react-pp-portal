...
class CampaignDetails extends Component {
  ...
  setDisplayed(data) {
    ...
    if (data) {
      ...
      if (this.props.details.days) {
        this.props.details.days.map(items => {
          const size = Object.keys(items).length;
          for (let i = 0; i < size; i++) {
            temp.push(items[i]);
          }
  ...
  fetchOriginal() {
    const tempOriginal = mockData.data;
    const indexOriginal = tempOriginal.indexOf(
      tempOriginal.find(({ id }) => id === this.props.id)
    );
    ...
    return (
      <React.Fragment>
        <Grid container spacing={3}>
          ...
          {this.state.displayed.map((item, key) => {
