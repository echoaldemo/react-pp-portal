...
class AddEventDialog extends React.Component {
  ...
  save = () => {
    ...
    if(this.state.content.length !== 0){
      ...
      if(this.state.contentError === false || this.state.errorCallBackDigit === false || this.state.errorPhoneNumber === false || this.state.errorTransferDigit === false){
  ...
  render() {
    const { classes, handleClose, open, audio, loaded } = this.props;
    return (
      <React.Fragment>
        <Dialog
          onClose={handleClose}
        ...
        >
          ...
          <DialogContent className={classes.content} style={{overflowX: 'auto', paddingBottom: 0}} id="style-1">
            <Typography className={classes.preSched}>Event</Typography>
            <DialogContentText className={classes.text} style={{ marginBottom: "15px" }}>
              ...
              <Grid item xs={12}>
                ...
                >
                  {this.state.types.map((option, i) => (
                    <MenuItem key={option.value} value={option.value} id={`selectTypeOfEvent${i}`}>
                      {option.label}
                    </MenuItem>
                  ))}
                </TextField>
            ...
            >
              <Grid item xs={12} sm={12} md={12} lg={12}>
                <TextField
                  ...
                  helperText={`Remaining characters: ${160-this.state.content.length}`}
                />
              </Grid>
            </Grid>
            <Divider light/>
            <Typography className={classes.preSched} style={{ marginTop: '16px' }}>Parameters</Typography>
            <DialogContentText className={classes.text} style={{ marginTop: '16px' }}>
            ...
            <React.Fragment>
              <Divider light/>
              <Typography className={classes.preSched} style={{ marginTop: '16px' }}>Live Audio</Typography>
              <DialogContentText className={classes.text}>
                Select the message that is played when the phone is answered. Remember to mention the Transfer and Do Not Call options and digits in this message
              </DialogContentText>
              ...
              <DialogContentText className={classes.text}>
                Enter a phone number to transfer the call to when the Transfer digit is pressed
              </DialogContentText>
              ...
              <div className={classes.audio}>
              { this.state.transferAudio.url !== '' || !this.state.transferAudio ? 
                <React.Fragment>
                  <DialogContentText className={classes.audioTitle}>
              ...
              : 
                <DialogContentText className={classes.optionText}>
                  <b>Option:</b> Select an audio to play when the transfer digit is pressed. 
                  <Link 
                    name="audioChooseFile"
                    href='#' 
              ...
              <DialogContentText className={classes.text}>
                Select the message that is played when the Callback digit it pressed. This message informs the costumer that they will receive a text message to schedule a callback.
              </DialogContentText>
              <Grid container spacing={3}>
                <Grid item xs={12} sm={6} md={6} lg={6}>
                  <TextField
                  ...
                  >
                    {this.state.digit.map((data, i) => 
                      <MenuItem key={data.name} id={`audioCallBack${i}`} value={data.value}>
                        {data.name}
                      </MenuItem>
                    )}
                  </TextField>
              ...
              <div className={classes.audio}>
              {this.state.callBackAudio.url !== '' || !this.state.callBackAudio ?
                <React.Fragment>
                  <DialogContentText className={classes.audioTitle}>
              ...
              : 
                <Button fullWidth variant="contained" className={classes.addAudio} id="chooseAudio" onClick={() => {
                  this.setState({
                    audio: true,
          ...
          <DialogActions className={classes.footer}>
            <Button onClick={handleClose} color="default" variant="contained" component="span" className={classes.cancel}>
              Cancel
            </Button>
            <Button disabled={this.state.btnSave} onClick={this.save} variant="contained" component="span" name="saveSMSCALL" className={classes.done}>
              Save
            </Button>
          </DialogActions>
        </Dialog>
