...
class MainAddNewAudio extends Component {
	...
	setNewAudioDetails = (unrec_uuid, unrecordedName, unrecordedPhrase, slug, phrasebook, selectedIndex) => {
		this.setState({
			unrec_uuid,
			unrecordedName,
			unrecordedPhrase,
	...
	handleSessionAdd = (blob, name, key, url, slug, phrasebook, recordingText, recordingName, recordingKey) => {
		let temp = {
		...
		setTimeout(() => {
			if (this.state.session.length !== 0) {
				...
				if (!push) {
					...
					this.props.showToast('caution', 'Error adding to session. Recording currently in session');
				}
				this.setState((prevState) => ({
		...
		if (recordingKey !== undefined && recordingName !== undefined && recordingText !== undefined) {
			this.nextIndex();
			this.setNewAudioDetails(recordingKey, recordingName, recordingText, slug, phrasebook);
			this.props.handleUnrecordedSelected(recordingKey);
		}
	};

	sessionUpload = (voice, version, modification, fadein, fadeout, convert, arr, audioType) => {
		let uploadStatArr = [];
		if (audioType === 'pitch') {
			arr.map((data, index) => {
				post(
	...
	handleRemoveSession = (audioKey) => {
		let newSession = this.state.session.splice(this.state.session.indexOf(audioKey), 1);
		this.setState({ session: newSession });
	};

	render() {
		const { classes, onClose, loading, voices, campaigns, versions, audio } = this.props;
						...
						<React.Fragment>
							<DialogTitle className={classes.dialogTitle}>
								<div className={classes.flex} style={{ color: '#fff' }}>
									...
									<IconButton id="close-add-new" className={classes.clearBtn} onClick={onClose}>
										<Clear />
									</IconButton>
								</div>
							</DialogTitle>
							...
							>
								<TabPanel value={this.state.value} index={0} style={{ padding: 30 }}>
									{/* Add New Audio */}
									<Grid container spacing={2}>
										{this.props.user_group === 10 ? (
											<React.Fragment>
												...
												<Grid item xs={12} sm={4} style={{ marginTop: '20px' }}>
													<FormControl variant="outlined" className={classes.formControl}>
														...
														<Select
															...
															input={
																<BootstrapInput // name="Select a voice"
												...
												<Grid item xs={12} sm={4} className={classes.selectMargin}>
													<FormControl variant="outlined" className={classes.formControl}>
														<InputLabel
															className={classes.label}
															htmlFor="age-customized-select"
														...
														<Select
															...
															onChange={(e) => {
																this.setNewAudioDetails(
																	...
																	parseInt(
																		e.currentTarget.getAttribute('selectedIndex')
																	)
																);
											...
											<React.Fragment>
												...
												<Grid container spacing={2}>
													...
													<Grid item xs={12} sm={5} className={classes.selectMargin}>
														<FormControl variant="outlined" className={classes.formControl}>
															<InputLabel
																className={classes.label}
																htmlFor="age-customized-select"
													...
													<Grid item sm={1} />
													<Grid item sm={1} />
													<Grid item xs={12} sm={5} className={classes.selectMargin2}>
														<FormControl variant="outlined" className={classes.formControl}>
															<InputLabel
																className={classes.label}
																htmlFor="age-customized-select"
													...
													<Grid item xs={12} sm={5} className={classes.selectMargin3}>
														<FormControl variant="outlined" className={classes.formControl}>
															<InputLabel
																className={classes.label}
																htmlFor="age-customized-select"
											...
											<Grid item xs={12} sm={6} style={{ marginTop: '15px' }}>
												{this.props.unrecordedSelected !== '' ? (
													<FormControlLabel
														...
														label={this.state.checked ? 'Record Audio' : 'Upload Audio'}
													/>
												) : null}
											</Grid>
										</Grid>
									...
									{this.state.checked ? (
										<Collapse in={this.state.checked}>
											...
											{this.props.user_group === 10 ? (this.props.campaignSelected &&
												this.props.versionSelected &&
												this.props.unrecordedSelected) !== '' ? (
												<RecordAudio
										...
										<React.Fragment>
											...
											<input
												...
												disabled={this.props.unrecordedSelected !== '' ? false : true}
											/>
											...
											<Grid item container justify="center">
												<Grid item xs={12} md={6}>
													...
													<FormHelperText className={classes.checkText}>
														Disable any modifications to the audio file upon upload
													</FormHelperText>
												</Grid>
												<Grid item xs={12} md={6}>
													<FormControlLabel
														...
														label="Convert"
														control={
															<Checkbox color="primary" checked={this.state.convert} />
														}
											...
											<Grid item container xs justify="center">
												<Grid item xs={12} md={6}>
													<FormControlLabel
														...
														label="Fade In"
														control={
															<Checkbox color="primary" checked={this.state.fadein} />
														}
												...
												<Grid item xs={12} md={6}>
													<FormControlLabel
														...
														label="Fade Out"
														control={
															<Checkbox color="primary" checked={this.state.fadeout} />
														}
								...
								<TabPanel value={this.state.value} index={1} className={classes.resSession}>
									{/* Session Upload */}
										...
										if (!this.state.checked) {
											...
											return (
												<Button
													...
													onClick={() => {
														if (audio[this.state.selectedIndex + 1] !== undefined) {
										...
										return (
											<Button
												...
												onClick={() => {
													...
													if (audioUrl === 'phrase') {
														this.sessionUpload(
															this.props.selectedVoice || this.props.voiceSelected,
															this.props.versionSelected,
															this.state.modification,
															this.state.fadein,
															this.state.fadeout,
